...
export function GoogleSignInButton({ className }: { className?: string }) {
  return (
    <Link
      href="/login"
      className={className ?? 'h-8 px-3 rounded-lg bg-card border border-border flex items-center gap-2 hover:bg-muted transition-colors'}
    >
      <GoogleIcon />
      <span className="text-foreground text-[11px] font-semibold">Sign in</span>
    </Link>
  )
}
